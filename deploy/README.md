...
代理地区在「代理管理 → 新增 / 编辑代理 → 请求地区」配置，绑定该代理的账号共用。
默认自动识别在「测试代理」或「保存并测试」时，通过代理访问出口 IP 与地区查询服务并缓存结果；
普通请求不实时查 IP。未识别、识别失败或选择「保留客户端地区」时不覆盖客户端日期、时区和搜索位置，
也可手动填写国家、地区、城市和 IANA 时区。轮换出口的代理需要重新测试或手动指定稳定地区，
缓存本身不能保证每次连接的出口都相同。

OpenAI HTTP 传输按固定依赖使用 native TLS，Linux 构建包含固定版本的 OpenSSL；
WebSocket 及自定义 CA 回退仍使用 rustls。保持现有证书校验和代理协议支持，不需要额外安装运行时
OpenSSL，也不新增 TLS 配置开关。
...
操作日志分别保留应用直连对端 IP 与代理上报 IP。经反向代理或 Docker 转发后，
直连 IP 可能始终是同一个网桥地址，这不代表所有操作来自同一位访客。
列表优先展示代理上报地址，并在下方保留直连地址；详情可核对两者。
代理上报地址按 `X-Forwarded-For` 的首个地址、`X-Real-IP`、`CF-Connecting-IP`
顺序选择有效 IP。上报值未经过可信代理验证，不用于鉴权，也不能单独作为真实来源证据。
若显示“未上报代理来源”，应检查入口反向代理是否正确设置来源头；历史日志缺失的地址无法补回。
对外入口应清理访客自行提交的来源头，并按实际代理链设置地址，避免仅透传可伪造的值。
...
本次启动按序应用 0006_users 与 0007_group_budgets；升级前保存数据库备份，并停止旧进程后启动新版。不要同时运行旧版与新版，也不要只回滚应用二进制：旧版不兼容新增表结构，如需回退应恢复升级前的数据库备份。

原管理员 ID、用户名及密码哈希不变；旧 Key 归属原管理员。每个旧 Key 会自动创建一个“迁移-…”分组，保留该 Key 原日/周额度、窗口、已用金额，以及当前获准账号的集合。原先不绑定分组的 Key 会纳入当时已有的账号，之后新增账号需显式加组。已有分组保留，可在后台逐步整理迁移组；把 Key 改到另一组后旧组账本仍保留，新请求按新组已有的本人额度计算。
...
v3.5.0 启动时自动执行 0008、0009 迁移。旧用户的共享并发/RPM 默认不限，旧 Key 的额外限制保留；管理员可在用户管理中设置共享上限。分组未配置的模型及升级前的请求按 1 倍计算，已有消费不会重算。新请求按当时配置的模型倍率计入消费和日/周额度。

升级前保存数据库备份。执行新迁移后，旧镜像会因缺少迁移而拒绝启动；如需回退，需恢复对应的升级前数据库备份，不能只换回旧镜像。版本镜像为 `ghcr.io/const-time/codex-proxy-rs:3.5.0`。

### v3.5.1 分组删除与应急处理

v3.5.1 自动执行 0010 迁移。已删除全部 Key 的分组可以在界面删除，历史额度、账本、请求记录及账号本身会保留，在途请求继续结算。仍绑定任何用户 Key（包括停用 Key）时返回冲突。升级前备份数据库，回退仍需恢复对应数据库备份。

v3.5.0 即使删除了 Key，也可能因迁移遗留的额度窗口而无法删除分组。优先升级至 v3.5.1；无需手动删除消费记录。若暂时不能升级，可在备份后停止应用、保持 PostgreSQL 运行，用 [应急 SQL](sql/cleanup-unused-group-window.sql) 清理指定分组的空窗口，再重启应用并通过界面删除分组。先查询真实 `grp_...` ID（不是分组显示名称），以 `psql -v ON_ERROR_STOP=1 -v group_id=grp_实际ID -f cleanup-unused-group-window.sql` 执行。

脚本仅在该组没有 Key、没有计费账本、没有未完成请求，且窗口日/周消费均为零时删除空窗口；其他情况不处理。不要清空表、禁用外键、删除 `_sqlx_migrations` 或更改迁移校验和。

### Codex 账号指纹与 Turn-state 接管

账号管理 → OpenAI 账号「更多操作 → 指纹 / Turn-state」有两个独立开关，**默认均关闭**：

- **指纹收敛**：最终账号确定后，按账号与客户端 Key 派生出站 session/thread/turn/window/cache 身份；
  保留 UUIDv7 时间戳与复合窗口序号，对齐已知请求头和 metadata，不改用户正文或本地会话亲和。
  开启后 WebSocket 握手不带 turn-state，每帧仍使用本轮状态。不是 TLS 指纹伪装，也不保证模型能力变化。
- **接管 turn-state**：还需要在「State 自动维护」开启全局维护，配置本账号的精确上游模型、
  启用的代理池和维护项。只对 HTTP 非同轮、非原生续接请求替换状态；不强制 WS 降级，不改 WS 帧状态。
  关闭、到期或没有合格候选时恢复原有账号作用域下的透传行为。

候选按账号、精确模型和凭据版本隔离，每项最多保留 3 个。独立探测候选需经过账号业务出口验证；
成功完成的非接管 HTTP 请求也可采集实际响应值。长度/块数同时为 0 时使用 292/10 块和 332/12 块
两种筛选形态，旧的显式筛选配置仍保留。默认 TTL 为签发时间起 1 小时，重复观测不续期；这是经验策略，
不是对上游有效期或能力的保证。返回 312/356、候选耗尽或到期不会因此禁用账号。
探测遇认证失败/限流会暂停维护项，不通过更换出口继续绕过。

请求明细分别记录实际发出的状态（客户端 / 接管）与实际返回值；响应未返回状态时显示请求值并标注来源。
普通用户接口不暴露原始状态。维护页只显示摘要和形态，代理密钥与候选在 `turn_state_management`
加密保存，配置写入带版本冲突检测和脱敏审计。实例间由维护任务同步配置，不保证瞬时同步或中断在途请求。
切换指纹模式或关闭账号接管会撤下该账号候选，建议空闲时切换；通用维护配置保存会重新验证候选。

首次升级会应用 `0022_turn_state_management`。先备份数据库，并保留所有实例共用的
`identity_hmac_secret` 文件；该密钥同时用于派生加密密钥，丢失或更换会使旧维护文档无法解密。
主动探测会消耗上游额度，默认不开启。
