...
运行设置 `GET /api/admin/settings` 返回 `subscriptionAutoResetEnabled`。`POST /api/admin/settings/update` 可传布尔值修改开关，省略则保留现值。默认开启：单账号组跟随上游，多账号组使用分组统一的独立七天周期。关闭时所有组使用独立周周期；切换不清空当前用量，手动重置仍可用。重新开启后首次观察只建立基准，不补做关闭期间的重置。
...
订阅管理仅重置明确选择的用户分组对，保留历史请求和计费账本。日窗口至下一个北京时间零点，手动重置不移动分组统一的周结束时间；同一 requestId 重试不会清除后续消费。重置与在途结算按数据库事务串行化，重置之后完成的请求仍计入配额。

自动联动开启时，账号主动重置成功只会重置该账号所在单账号分组下用户的日/周额度。多账号分组（包含停用成员）采用独立、统一的七天周期，不随某个成员重置；新订阅沿用分组周期。上游自行重置和自然恢复通过已持久化的配额快照识别：仅 `window_seconds = 604800` 的账号级周窗口，其刷新边界向后推进超过 60 秒才触发。5 小时等短周期、比例下降和周边界未变化均不触发。首次观察只建立基线，重复、过期快照及重复兑换不重复重置；后台每 60 秒检查一次已取得的上游配额，账号页面读取新配额时也会检查。自然窗口有明确边界时，保留边界之后已经产生的消费。单账号组在未确认上游换窗时保留并累计用量，不按本地七天周期清零。独立周换窗和日换窗分别记录 group_period、daily_period，包含时间和受影响订阅；只读列表不会虚拟清零。识别时效取决于上游配额刷新；未被快照观察到的外部重置无法可靠识别。
...
账号详情的“模型使用排行”标题旁提供独立额度预测弹窗。它直接读取同一账号额度窗口的预测数据，
按实际返回的窗口切换，保留原始费用与倍率后计费，以及预计/剩余 Token。打开弹窗不触发上游查询；
点击“刷新额度”复用账号额度刷新接口并同步更新展开区。费用区间与周期 Token 的范围分别标注。
...
OpenAI 新请求会记录 `upstream.location.applied`（选定账号的代理位置策略及实际改写数量）与
`upstream.request.profile`（发送前的传输、压缩、白名单请求头、位置字段和会话摘要）。
代理快照包含 `proxyId / mode / detectedIp / detectedAt / location`；检测 IP 是最近一次代理测试结果，
不是每次请求重新探测的出口。地区配置和准备的请求头不能证明已发送、上游接受或实际 TLS 指纹一致；
需结合对应 attempt/exchange 的连接、发送与响应事件，尤其区分 WebSocket 连接复用。
会话和路由标识保存 SHA-256，认证与 Cookie 仅保存存在性；不会写入代理 URL、密码或完整正文。
请求头白名单值最多 128 字符，结构化位置每类最多两处且字段最多 48 字符，仍受时间线整体大小限制。
无需启用完整报文日志，管理员可在“请求详情 → 请求诊断 → 上游请求参数”及导出诊断包查看。
界面的出口检测时间按该请求快照中的有效时区显示，并标注时区及 UTC 偏移；无法转换时保留原始时间并说明原因。
请求详情上方标注 UTC+8；上游参数另外展示所选诊断请求的 `createdAt` 在各次尝试保存的代理时区中的时间。
“上次出口检测时间”独立展示代理探测时刻，不作为请求发生时间。
时间线与导出诊断包中的 `detectedAt` 原值保持不变。
旧记录和其他尚未采集的平台不会通过当前配置补写历史。
