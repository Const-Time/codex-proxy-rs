...
async fn quota_forecast_excludes_openai_prewarm_but_preserves_inference_and_audit() {
    ...
    // Billing uses persisted per-request amounts; never reconstruct it from today's multiplier.
    sqlx::query("update model_requests set billing_multiplier = case when id = 'normal' then 2 else 3 end where id in ('normal', 'review')")
        .execute(&database.pool).await.unwrap();
    let billed = store.load_quota_forecast_history(&query).await.unwrap();
    assert_eq!(billed.usage.billed_usd, 5.0);
    assert_eq!(billed.usage.missing_billed_count, 0);
    ...
    let detail = repository
        .usage_record_detail("prewarm", None)
        .await
        .unwrap();
    ...
    assert_eq!(history.usage.billed_usd, 2.0);
    assert_eq!(history.usage.missing_billed_count, 1);
    assert_eq!(history.usage.excluded_request_count, 1);
