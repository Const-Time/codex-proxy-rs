...
pub(super) async fn load_history(
    ...
    for row in &rows {
        let usage = QuotaForecastUsage {
            ...
            billed_usd: window_usage_value(row, "billed_usd")?,
            missing_billed_count: window_usage_count(row, "missing_billed_count")?,
    ...
    format!(
        "with scoped as materialized (
            select mr.id, mr.started_at, mr.completed_at,
                   ...
                   mr.cost_amount, mr.cost_currency, mr.billed_cost_amount,
            ...
            select *,
                ...
                case when included and settled and known_cost then coalesce(billed_cost_amount, 0) else 0 end as billed_usd,
                (included and settled and (not known_cost or billed_cost_amount is null))::integer as missing_billed,
            ...
            select id, started_at, completed_at, has_document,
                ...
                sum(billed_usd) over w as billed_usd,
                sum(missing_billed) over w as missing_billed_count,
        ...
        select false as is_total, s.started_at, s.completed_at,
            ...
            s.usd::double precision, s.billed_usd::double precision, s.missing_billed_count::bigint, s.excluded_request_count::bigint,
        ...
        select true, $3, $3,
            ...
            coalesce(sum(usd), 0)::double precision,
            coalesce(sum(billed_usd), 0)::double precision, coalesce(sum(missing_billed), 0)::bigint, coalesce(sum(excluded), 0)::bigint,
