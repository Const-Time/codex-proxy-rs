...
impl QuotaForecastUsage {
    ...
    fn difference(&self, baseline: &Self) -> Option<Self> {
        ...
        let billed_usd = self.billed_usd - baseline.billed_usd;
        if !billed_usd.is_finite() || billed_usd < -1e-9 {
            return None;
        }
        ...
        Some(Self {
            ...
            usd: usd.max(0.0),
            billed_usd: billed_usd.max(0.0),
            missing_billed_count: self
                .missing_billed_count
                .checked_sub(baseline.missing_billed_count)?,
