...
`0018_proxy_request_location.sql` 为共享代理增加地区策略及最近测试位置，账号通过绑定投影有效配置。
`0019_proxy_request_context.sql` 增加只读投影函数，将策略来源、代理 ID 和最近出口检测信息与有效位置一并读取，供请求诊断保存当次快照；不回填历史请求。
旧 `0017` 估算表保留原迁移，不再由新版估算器读写；升级不改写历史请求费用或已用额度。
...
0023 新增分组级订阅周周期和带记录的自动换窗。单账号组按已确认上游重置联动，多账号组独立统一七天；不自动恢复旧版错误清零的历史额度。迁移不修改原有账本与窗口，详见用户订阅文档。

0024 将系统 State 探测迁移到独立记录表，只迁出带完整系统标记的探测，不修改业务请求与账本。
旧记录缺少的候选结果、出口地区保持未知，原始 State 不复制。回滚旧二进制会重新写入业务请求表，升级前请备份且不要混跑旧版本。
