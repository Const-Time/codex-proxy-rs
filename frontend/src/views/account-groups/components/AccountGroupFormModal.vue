...
import { useGroupModelOptions } from '../composables/useGroupModelOptions'
...
const { options: modelOptions, loading: loadingModels, message: modelMessage, reload: reloadModels } = useGroupModelOptions(open, () => props.group)
    ...
    <BaseForm class="grid gap-5">
      ...
      <div class="grid gap-3" role="group" aria-label="模型计费倍率">
        <div class="flex items-center justify-between gap-3">
          ...
          <BaseButton size="sm" :loading="loadingModels" :disabled="saving || !group" @click="reloadModels">
            刷新模型
          </BaseButton>
          <BaseButton size="sm" :disabled="saving || loadingModels || !modelOptions.length || form.modelMultipliers.length >= 200" @click="form.modelMultipliers.push({ model: '', multiplier: '1' })">
            添加模型
          </BaseButton>
        </div>
        <p class="m-0 text-cp-sm text-cp-text-secondary">
        ...
        <p v-if="modelMessage" role="status" class="m-0 text-cp-sm text-cp-text-secondary">
          {{ modelMessage }}
        </p>
        <div v-for="(row, index) in form.modelMultipliers" :key="index" class="grid grid-cols-[minmax(0,1fr)_6rem_auto] items-center gap-2">
          <BaseSelect v-model="row.model" :aria-label="`模型名称 ${index + 1}`" :options="modelOptions.filter(option => option.value === row.model || !form.modelMultipliers.some(other => other.model === option.value))" :placeholder="loadingModels ? '加载模型中...' : '选择分组账号的模型'" :disabled="saving || loadingModels" />
